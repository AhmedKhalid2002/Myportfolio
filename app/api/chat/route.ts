...
const getMockResponse = (message: string, language: string): string => {
  ...
  if (language === 'ar') {
    if (lowerMessage.includes('مهارة') || lowerMessage.includes('skill')) {
      ...
      return `لقد طورت عدة مشاريع رائعة:\n\n${portfolioData.projects.slice(0, 3).map(p => `📌 **${p.title_ar || p.title}**: ${p.description_ar || p.description}`).join('\n\n')}`;
    } else if (lowerMessage.includes('تجربة') || lowerMessage.includes('experience')) {
      return `خبرتي المهنية:\n\n${portfolioData.experience.slice(0, 3).map(e => `🏢 **${e.role_ar || e.role}** في ${e.company_ar || e.company}\n${e.duration}\n${e.desc_ar || e.desc}`).join('\n\n')}`;
    ...
    if (lowerMessage.includes('skill')) {
      ...
      return `My professional experience:\n\n${portfolioData.experience.slice(0, 3).map(e => `🏢 **${e.role}** at ${e.company}\n${e.duration}\n${e.desc}`).join('\n\n')}`;
