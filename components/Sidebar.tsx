...
// تعريف هيكل العنصر ليشمل الترجمات
interface NavItem {
  id: string;
  label_en: string;
  label_ar: string;
...
export default function Navigation({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed ${dir === 'rtl' ? 'right-0 border-l' : 'left-0 border-r'} left-0 top-0 h-screen w-20 hidden xl:flex flex-col items-center justify-between py-8 border-r backdrop-blur-md z-50 transition-all duration-500 ${
          isDarkMode
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {/* Tooltip */}
                <span
                  className={`absolute left-16 scale-0 group-hover:scale-100 opacity-0 group-hover:opacity-100 px-3 py-1.5 rounded-lg text-white text-xs font-bold whitespace-nowrap shadow-md transition-all duration-300 ${
                    isDarkMode
                      ? 'bg-cyan-600'
      ...
      <nav
        className={`xl:hidden fixed bottom-4 left-4 right-4 rounded-2xl border backdrop-blur-md z-50 p-3 flex justify-around items-center transition-all duration-500 ${
          isDarkMode
            ? 'bg-slate-950/90 border-slate-800 shadow-xl shadow-cyan-900/10'
